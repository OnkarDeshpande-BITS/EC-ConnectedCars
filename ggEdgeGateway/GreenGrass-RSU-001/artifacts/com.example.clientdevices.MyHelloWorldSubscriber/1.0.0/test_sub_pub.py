...
def on_hello_world_message(event):
    try:
        ...
        binary_message = BinaryMessage(message=bytes("test", 'utf-8'))
        publish_message = PublishMessage(binary_message=binary_message)
        ipc_pub_client = GreengrassCoreIPCClientV2()
        ipc_pub_client.publish_to_topic(topic='edge/GG-Rsu/alerts/situation', publish_message=publish_message)
